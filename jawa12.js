...
const args = {
    ...
    // Options/flags
    randomPath: process.argv.includes('--random'),
    randomReferer: process.argv.includes('--random') || process.argv.includes('--referer'),
...
function generateWindowsHeaders(browserVersion, wfwf) {
    ...
    // Use bypass user agents for better Cloudflare bypass
    const userAgent = args.bypass ?
        bypassUserAgents[Math.floor(Math.random() * bypassUserAgents.length)] :
