            ...
            const intervalId = setInterval(async () => {
                ...
                const sendRequests = async () => {
                    while (true) {
                        ...
                        const request = client.request(randomizedHeaders, {
                            endStream: true
                        });
...
/*                            const status = res2[':status'] || res2.statusCode;

                            if (status === 429) {
                              console.log('[!] RATE LIMIT DETECTED (429), exiting...');
                              process.exit(1); // Langsung hentikan program
                            }
...
/*                        const status = res2[':status'] || res2.statusCode;
                        ...
                        if (status === 429) {
                          console.log('[INFO] RATE LIMIT DETECTED (429), exiting...');
                          process.exit(1); // Langsung hentikan program
                        }
                        console.log(`[INFO] Status ${status}`);*/
                ...
                await Promise.all(requests1.map(({
                    requestPromise
                }) => requestPromise));
