...
func worker(proxyQueue <-chan string, workerID int) {
	...
	for proxy := range proxyQueue {
		...
		if result != nil {
			...
			fmt.Printf("[W%02d] [LIVE] %s | %s-%s | %dms | %s | HTTPS:%s | Google:%s\n",
				workerID, proxy, result.Country, result.City, result.Speed,
				result.Anonymity, httpsStatus, googleStatus)
			mutex.Unlock()
		} else {
			// Jika proxy mati, tidak perlu di-print agar terminal tidak penuh (optional).
			// Jika ingin ditampilkan, hapus tanda // pada baris di bawah ini:
			// fmt.Printf("[W%02d] [DEAD] %s | Failed\n", workerID, proxy)
		}
	}
}
...
func checkProxy(proxy string) *ProxyInfo {
	...
	// PERBAIKAN: Gunakan IP asli proxy (variabel 'ip') bukan dari ipResponse.Origin
	ipInfo := getIPInfo(ip)
